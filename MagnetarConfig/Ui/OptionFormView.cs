...
internal sealed class OptionFormView : Window, IAutoSaveContent
{
        ...
        Action editMods = null) : base(title)
    {
        ...
        {
            Add(new Label(banner) { X = 1, Y = 0, Width = Dim.Fill(1), ColorScheme = TerminalTheme.Window });
            top = 1;
        }
        ...
        var filterLabel = new Label("Filter: ") { X = 1, Y = top, ColorScheme = TerminalTheme.Window };
    ...
    private void RebuildForm()
    {
        ...
        else
        {
            ...
            if (matches.Count == 0)
            {
                form.Add(new Label("(no settings match)") { X = 0, Y = 0, ColorScheme = TerminalTheme.Window });
                y = 1;
            }
            foreach (IGrouping<string, OptionDefinition> group in matches.GroupBy(o => o.Category))
            {
                if (y > 0)
                    y += 1;
                form.Add(new Label(group.Key + " /") { X = 0, Y = y, Width = Dim.Fill(), ColorScheme = TerminalTheme.Window });
                y += 1;
    ...
    private View BuildEditor(OptionDefinition def, int x)
    {
        switch (def.Kind)
        {
            ...
            case OptionKind.StringList:
            {
                ...
                return new Label("(edited elsewhere)") { X = x, Y = 0, ColorScheme = TerminalTheme.Window };
            }
            default:
            {
    ...
    private void OnFieldEdited(OptionDefinition def, View editor, string text)
    {
        ...
        {
            invalid.Remove(def);
            editor.ColorScheme = TerminalTheme.Window;
        ...
        {
            invalid.Add(def);
            editor.ColorScheme = TerminalTheme.Error;
        }
