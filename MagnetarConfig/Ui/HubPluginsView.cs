...
internal sealed class HubPluginsView : Window
{
    ...
    public HubPluginsView(string magnetarConfigDir, AtomicFile writer) : base("Hub Plugins")
    {
        ...
        {
            X = 0, Y = 0, Width = Dim.Percent(34), Height = Dim.Fill(2), ColorScheme = TerminalTheme.Window,
        };
        var filterLabel = new Label("Filter: ") { X = 0, Y = 0 };
        filter = new TextField(string.Empty)
        { X = Pos.Right(filterLabel), Y = 0, Width = Dim.Fill(), ColorScheme = TerminalTheme.Window };
        filter.TextChanged += _ => ApplyFilter();
        list = new ListView(Array.Empty<string>())
        { X = 0, Y = 1, Width = Dim.Fill(), Height = Dim.Fill(), ColorScheme = TerminalTheme.Window };
        ...
        {
            X = Pos.Percent(34), Y = 0, Width = Dim.Fill(), Height = Dim.Fill(2), ColorScheme = TerminalTheme.Window,
        };
