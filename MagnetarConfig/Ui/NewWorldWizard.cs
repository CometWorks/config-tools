...
internal static class NewWorldWizard
{
    ...
    {
        var dlg = new Dialog("Pick a template", 60, 18) { ColorScheme = TerminalTheme.Dialog };
        var list = new ListView(instance.Templates.Templates.Select(t => $"{t.DisplayName}   ({t.FolderName})").ToList())
        {
            X = 1, Y = 1, Width = Dim.Fill(2), Height = Dim.Fill(3), ColorScheme = TerminalTheme.Dialog,
        };
    ...
    {
        var dlg = new Dialog("World name", 56, 10) { ColorScheme = TerminalTheme.Dialog };
        var field = new TextField(template.DisplayName) { X = 1, Y = 2, Width = Dim.Fill(2) };
        var warn = new Label("") { X = 1, Y = 4, Width = Dim.Fill(2), ColorScheme = TerminalTheme.Dialog };
        string result = null;
