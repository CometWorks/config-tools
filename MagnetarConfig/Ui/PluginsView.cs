...
internal sealed class PluginsView : Window
{
    ...
    public PluginsView(string magnetarConfigDir, AtomicFile writer, ToolSettings settings) : base("Plugins")
    {
        ...
        {
            X = 0, Y = 0, Width = Dim.Percent(50), Height = Dim.Fill(2), ColorScheme = TerminalTheme.Window,
        };
        localList = new ListView(Array.Empty<string>())
        { X = 0, Y = 0, Width = Dim.Fill(), Height = Dim.Fill(), ColorScheme = TerminalTheme.Window };
        ...
        {
            X = Pos.Percent(50), Y = 0, Width = Dim.Fill(), Height = Dim.Fill(2), ColorScheme = TerminalTheme.Window,
        };
