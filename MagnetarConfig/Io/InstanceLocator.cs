...
internal static class InstanceLocator
{
    ...
    /// nothing here assumes a deploy location. AppContext resolves the
    /// executable directory for single-file builds and the managed application
    /// directory when hosted by <c>dotnet MagnetarConfig.dll</c> (tests, IDE).
    /// </summary>
    public static string InstallRoot => Path.TrimEndingDirectorySeparator(
