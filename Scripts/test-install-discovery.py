    ...
    with tempfile.TemporaryDirectory(prefix='config-discovery-') as directory:
        ...
        for returning in (False, True):
            ...
            if pid == 0:
                ...
                os.chdir(root if returning else install)
            ...
            def expect(text):
                ...
                if returning:
                    assert b'Installations' not in output, 'Returning user saw the startup picker'
                output.clear()
            ...
            try:
                ...
                os.waitpid(pid, 0)
                os.close(fd)
    print(f'PASS: {product} discovers an unpacked install outside the tool, opens it, and skips the picker on restart.')
