# MagnetarConfig — Design and implementation notes

Technical companion to the **[MagnetarConfig user manual](MagnetarConfig.md)**.
...
> [user manual](MagnetarConfig.md); they are intentionally absent here.
...
[user manual](MagnetarConfig.md#2-running-magnetarconfig)):
...
├── MagnetarConfig.xml                  ← this tool's own settings (see the manual)
...
[Configuration.md](https://github.com/CometWorks/magnetar/blob/main/Docs/Configuration.md)).
...
- **Magnetar logs** — `info.log` is the current launch's log; the launcher
  ...
  [Configuration.md](https://github.com/CometWorks/magnetar/blob/main/Docs/Configuration.md)). The reader treats the file named by
...
```
MagnetarConfig/                      → executable "MagnetarConfig"
├── MagnetarConfig.csproj            net10.0 (Linux + Windows)
...
├── Ui/
│   ├── TerminalTheme.cs          ColorSchemes + border styles
...
└── State/
    └── ToolSettings.cs              MagnetarConfig.xml: last plugin-manifest folder

MagnetarConfigTests/
└── MagnetarConfigTests.csproj       xUnit, net10.0, fixture files
```
...
New xUnit project `MagnetarConfigTests` (patterned on `PluginSdkTests`):
...
Standalone config-tools releases publish with `SelfContained=true`,
`PublishSingleFile=true`, and `IncludeNativeLibrariesForSelfExtract=true`.
Trimming is disabled because the UI and serializers use reflection. Each
...
`MagnetarConfigTests`. What that covers, and the few gaps:
...
- **Not implemented** — the `ExternalChangeWatcher` external-change / conflict
  ...
  ([user manual](MagnetarConfig.md#2-running-magnetarconfig)) — not recent instance pairs,
