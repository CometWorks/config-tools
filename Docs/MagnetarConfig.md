...
**[MagnetarConfig — design and implementation notes](MagnetarConfigInternals.md)**.
...
One window at a time sits on the plain dark desktop, with a menu bar on top
...
[save-pipeline state machine](MagnetarConfigInternals.md#9-state-machines) for
the full flow.
...
- `MagnetarConfig.xml` — the tool's own small settings file.
